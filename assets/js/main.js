...
(function ($) {
	...
	jQuery(function () {
		...
		function initSplitTextAnimation() {
			...
			$animatedTexts.each(function () {
				...
				let folksGradient = $el.hasClass("custom-gradient")
					? chroma.scale(["#00E5A0", "#60A5FA"])
					: chroma.scale(["#60A5FA", "#ffae00"]);
		...
		$('#downloadCV').on('click', function (e) {
			...
			const link = $('<a></a>')
				.attr('href', '/assets/document/resume.pdf') // Your PDF path
				.attr('download', 'My_Resume.pdf')
				.appendTo('body');
		...
		$('#contactForm').on('submit', function (e) {
			...
			const btnText = submitBtn.find('.btn-animated-text');
			...
			btnText.html('<span class="spinner"></span> Sending...');
